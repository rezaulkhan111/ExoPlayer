...
/** FFmpeg audio decoder.  */ /* package */
class FfmpegAudioDecoder(
        format: Format,
        numInputBuffers: Int,
        numOutputBuffers: Int,
        initialInputBufferSize: Int,
        outputFloat: Boolean) : SimpleDecoder<DecoderInputBuffer?, SimpleDecoderOutputBuffer?, FfmpegDecoderException?>(arrayOfNulls(numInputBuffers), arrayOfNulls(numOutputBuffers)) {
    private val codecName: String
    private val extraData: ByteArray?
    private val encoding: @PcmEncoding Int
    ...
    override fun decode(
            inputBuffer: DecoderInputBuffer, outputBuffer: SimpleDecoderOutputBuffer, reset: Boolean): FfmpegDecoderException? {
        if (reset) {
    ...
    fun getEncoding(): @PcmEncoding Int {
        return encoding
    }
    ...
    private external fun ffmpegInitialize(
            ...
            outputFloat: Boolean,
            rawSampleRate: Int,
            rawChannelCount: Int): Long

    private external fun ffmpegDecode(
            context: Long, inputData: ByteBuffer, inputSize: Int, outputData: ByteBuffer, outputSize: Int): Int
