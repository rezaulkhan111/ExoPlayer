...
class StarRating : Rating {
    ...
    /**
     * Creates a unrated instance with `maxStars`. If `maxStars` is not a positive
     * integer, it will throw an [IllegalArgumentException].
     *
     * @param maxStars The maximum number of stars this rating can have.
     */
    constructor(@IntRange(from = 1) maxStars: Int) {
        Assertions.checkArgument(maxStars > 0, "maxStars must be a positive integer")
        this.maxStars = maxStars
        starRating = Rating.Companion.RATING_UNSET
    }

    /**
     * Creates a rated instance with `maxStars` and the given fractional number of stars.
     * Non-integer values may be used to represent an average rating value. If `maxStars` is not
     * a positive integer or `starRating` is out of range, it will throw an [ ].
     *
     * @param maxStars The maximum number of stars this rating can have.
     * @param starRating A fractional number of stars of this rating from `0f` to `maxStars`.
     */
    constructor(@IntRange(from = 1) maxStars: Int, @FloatRange(from = 0.0) starRating: Float) {
        Assertions.checkArgument(maxStars > 0, "maxStars must be a positive integer")
    ...
    override val isRated: Boolean
        get() {
            return starRating != Rating.Companion.RATING_UNSET
        }
    ...
    public override fun equals(obj: Any?): Boolean {
        if (!(obj is StarRating)) {
            return false
        }
        val other: StarRating = obj
    ...
    @Target(TYPE_USE)
    @IntDef([Rating.Companion.FIELD_RATING_TYPE, StarRating.Companion.FIELD_MAX_STARS, StarRating.Companion.FIELD_STAR_RATING])
    private annotation class FieldNumber constructor()

    public override fun toBundle(): Bundle {
        val bundle: Bundle = Bundle()
        bundle.putInt(StarRating.Companion.keyForField(Rating.Companion.FIELD_RATING_TYPE), StarRating.Companion.TYPE)
        bundle.putInt(StarRating.Companion.keyForField(StarRating.Companion.FIELD_MAX_STARS), maxStars)
        bundle.putFloat(StarRating.Companion.keyForField(StarRating.Companion.FIELD_STAR_RATING), starRating)
        return bundle
    }

    companion object {
        // Bundleable implementation.
        private val TYPE: @RatingType Int = Rating.Companion.RATING_TYPE_STAR
        private val MAX_STARS_DEFAULT: Int = 5
        private val FIELD_MAX_STARS: Int = 1
        private val FIELD_STAR_RATING: Int = 2

        /** Object that can restore a [StarRating] from a [Bundle].  */
        val CREATOR: Bundleable.Creator<StarRating> = Bundleable.Creator({ bundle: Bundle? -> StarRating.Companion.fromBundle(bundle) })
        private fun fromBundle(bundle: Bundle): StarRating {
            Assertions.checkArgument((
                    bundle.getInt(StarRating.Companion.keyForField(Rating.Companion.FIELD_RATING_TYPE),  /* defaultValue= */Rating.Companion.RATING_TYPE_UNSET)
                            == StarRating.Companion.TYPE))
            val maxStars: Int = bundle.getInt(StarRating.Companion.keyForField(StarRating.Companion.FIELD_MAX_STARS),  /* defaultValue= */StarRating.Companion.MAX_STARS_DEFAULT)
            val starRating: Float = bundle.getFloat(StarRating.Companion.keyForField(StarRating.Companion.FIELD_STAR_RATING),  /* defaultValue= */Rating.Companion.RATING_UNSET)
            return if (starRating == Rating.Companion.RATING_UNSET) StarRating(maxStars) else StarRating(maxStars, starRating)
        }

        private fun keyForField(field: @StarRating.FieldNumber Int): String {
