...
interface SequenceableLoader {
    ...
    interface Callback<T : SequenceableLoader?> {
        /**
         * Called by the loader to indicate that it wishes for its [.continueLoading] method
         * to be called when it can continue to load data. Called on the playback thread.
         */
        fun onContinueLoadingRequested(source: T)
    }
     ...
     * [C.TIME_END_OF_SOURCE] if the data is fully buffered.
     */
     ...
     */
    fun continueLoading(positionUs: Long): Boolean
     ...
     *
     *
     * Re-evaluation may discard buffered media or cancel ongoing loads so that media can be
     ...
     */
    fun reevaluateBuffer(positionUs: Long)
}