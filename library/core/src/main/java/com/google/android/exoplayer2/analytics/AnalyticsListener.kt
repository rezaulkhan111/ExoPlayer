...
interface AnalyticsListener {
     ...
     * A set of [EventFlags].
     */
    class Events(flags: FlagSet, eventTimes: SparseArray<EventTime?>) {
        ...
        init {
            this.flags = flags
            val flagsToTimes: SparseArray<EventTime> = SparseArray<EventTime>( /* initialCapacity= */flags.size())
            for (i in 0 until flags.size()) {
                @EventFlags val eventFlag: Int = flags[i]
    ...
    class EventTime {
        ...
        /**
         * @param realtimeMs Elapsed real-time as returned by `SystemClock.elapsedRealtime()` at
         * the time of the event, in milliseconds.
         * @param timeline Most recent [Timeline] that contains the event position.
         * @param windowIndex Window index in the `timeline` this event belongs to, or the
         * prospective window index if the timeline is not yet known and empty.
         * @param mediaPeriodId [Media period identifier][MediaPeriodId] for the media period this
         * event belongs to, or `null` if the event is not associated with a specific media
         * period.
         * @param eventPlaybackPositionMs Position in the window or ad this event belongs to at the time
         * of the event, in milliseconds.
         * @param currentTimeline The current [Timeline] at the time of the event (equivalent to
         * [Player.getCurrentTimeline]).
         * @param currentWindowIndex The current window index in `currentTimeline` at the time of
         ...
         * @param currentMediaPeriodId [Media period identifier][MediaPeriodId] for the currently
         * playing media period at the time of the event, or `null` if no current media period
         ...
         * @param totalBufferedDurationMs Total buffered duration from `currentPlaybackPositionMs`
   ...
   * #onPlayWhenReadyChanged(EventTime, boolean, int)} instead.""")
     ...
     */
    fun onPlaybackStateChanged(eventTime: EventTime?, @Player.State state: Int) {}
    ...
    fun onPlayWhenReadyChanged(
            eventTime: EventTime?, playWhenReady: Boolean, @Player.PlayWhenReadyChangeReason reason: Int) {
    }
    ...
    fun onPlaybackSuppressionReasonChanged(
            eventTime: EventTime?, @Player.PlaybackSuppressionReason playbackSuppressionReason: Int) {
    }
     ...
     */
    fun onTimelineChanged(eventTime: EventTime?, @Player.TimelineChangeReason reason: Int) {}
   ...
   * Player.PositionInfo, int)} instead.""")
    fun onPositionDiscontinuity(eventTime: EventTime?, @Player.DiscontinuityReason reason: Int) {
    }
   ...
   * int)} with reason {@link Player#DISCONTINUITY_REASON_SEEK} instead.""")
     ...
     */
    fun onRepeatModeChanged(eventTime: EventTime?, @Player.RepeatMode repeatMode: Int) {}
     ...
     */
    fun onAvailableCommandsChanged(eventTime: EventTime?, availableCommands: Player.Commands?) {}
     ...
     */
    fun onDrmSessionAcquired(eventTime: EventTime?, state: @DrmSession.State Int) {}
    ...
    companion object {
         ...
         */
        const val EVENT_PLAYBACK_SUPPRESSION_REASON_CHANGED: Int = Player.EVENT_PLAYBACK_SUPPRESSION_REASON_CHANGED
         ...
         */
        const val EVENT_SHUFFLE_MODE_ENABLED_CHANGED: Int = Player.EVENT_SHUFFLE_MODE_ENABLED_CHANGED
         ...
         */
        const val EVENT_SEEK_FORWARD_INCREMENT_CHANGED: Int = Player.EVENT_SEEK_FORWARD_INCREMENT_CHANGED

        /**
         * [Player.getMaxSeekToPreviousPosition] changed.
         */
        const val EVENT_MAX_SEEK_TO_PREVIOUS_POSITION_CHANGED: Int = Player.EVENT_MAX_SEEK_TO_PREVIOUS_POSITION_CHANGED

        /**
         * [Player.getTrackSelectionParameters] changed.
         */
        const val EVENT_TRACK_SELECTION_PARAMETERS_CHANGED: Int = Player.EVENT_TRACK_SELECTION_PARAMETERS_CHANGED
         ...
         */
        const val EVENT_SKIP_SILENCE_ENABLED_CHANGED: Int = Player.EVENT_SKIP_SILENCE_ENABLED_CHANGED
