...
object Util {
    ...
    fun <T> removeRange(list: List<T>, fromIndex: Int, toIndex: Int) {
        ...
        require(((fromIndex < 0 || toIndex > list.size) || fromIndex) <= toIndex)
    ...
    fun <T> nullSafeArrayCopy(input: Array<T>?, length: Int): Array<T> {
        Assertions.checkArgument(length <= input!!.size)
    ...
    fun <T> nullSafeArrayCopyOfRange(input: Array<T>, from: Int, to: Int): Array<T> {
        ...
        Assertions.checkArgument(to <= input.size)
    ...
    fun <T> nullSafeArrayAppend(original: Array<T>?, newElement: T): Array<T> {
        val result = Arrays.copyOf(original, original!!.size + 1)
    ...
    fun <T> nullSafeListToArray(list: List<T>, array: Array<T>) {
        Assertions.checkState(list.size == array.size)
    ...
    fun binarySearchCeil(
        ...
        return if (stayInBounds) Math.min(array.size - 1, index) else index
    }
    ...
    fun <T : Comparable<T>?> binarySearchCeil(
        ...
        return if (stayInBounds) Math.min(list.size - 1, index) else index
    }
    ...
    fun parseXsDuration(value: String): Long {
        ...
        return if (matcher.matches()) {
            ...
            var durationSeconds: Double = if (years != null) years.toDouble() * 31556908 else 0
            val months = matcher.group(5)
            durationSeconds += if (months != null) months.toDouble() * 2629739 else 0
            val days = matcher.group(7)
            durationSeconds += if (days != null) days.toDouble() * 86400 else 0
            val hours = matcher.group(10)
            durationSeconds += if (hours != null) hours.toDouble() * 3600 else 0
            val minutes = matcher.group(12)
            durationSeconds += if (minutes != null) minutes.toDouble() * 60 else 0
            val seconds = matcher.group(14)
            durationSeconds += seconds?.toDouble() ?: 0
    ...
    fun getMediaDurationForPlayoutDuration(playoutDuration: Long, speed: Float): Long {
        return if (speed == 1f) {
            playoutDuration
        } else Math.round(playoutDuration.toDouble() * speed)
    }
     ...
     */
    fun getPcmEncoding(bitDepth: Int): @PcmEncoding Int {
     ...
     */
    fun isEncodingLinearPcm(encoding: @C.Encoding Int): Boolean {
     ...
     */
    fun isEncodingHighResolutionPcm(encoding: @PcmEncoding Int): Boolean {
    ...
    fun getPcmFrameSize(pcmEncoding: @PcmEncoding Int, channelCount: Int): Int {
        return when (pcmEncoding) {
            ...
            C.ENCODING_PCM_24BIT -> channelCount * 3
            C.ENCODING_PCM_32BIT, C.ENCODING_PCM_FLOAT -> channelCount * 4
            C.ENCODING_INVALID, Format.Companion.NO_VALUE -> throw IllegalArgumentException()
            else -> throw IllegalArgumentException()
        }
    }

    /** Returns the [C.AudioUsage] corresponding to the specified [C.StreamType].  */
    fun getAudioUsageForStreamType(streamType: @C.StreamType Int): @AudioUsage Int {
        return when (streamType) {
            C.STREAM_TYPE_ALARM -> C.USAGE_ALARM
            C.STREAM_TYPE_DTMF -> C.USAGE_VOICE_COMMUNICATION_SIGNALLING
            C.STREAM_TYPE_NOTIFICATION -> C.USAGE_NOTIFICATION
            C.STREAM_TYPE_RING -> C.USAGE_NOTIFICATION_RINGTONE
            C.STREAM_TYPE_SYSTEM -> C.USAGE_ASSISTANCE_SONIFICATION
            C.STREAM_TYPE_VOICE_CALL -> C.USAGE_VOICE_COMMUNICATION
            C.STREAM_TYPE_MUSIC -> C.USAGE_MEDIA
            else -> C.USAGE_MEDIA
        }
    }
    ...
    fun getAudioContentTypeForStreamType(
        ...
        return when (streamType) {
            ...
            C.STREAM_TYPE_MUSIC -> C.AUDIO_CONTENT_TYPE_MUSIC
            else -> C.AUDIO_CONTENT_TYPE_MUSIC
        }
    }

    /** Returns the [C.StreamType] corresponding to the specified [C.AudioUsage].  */
    fun getStreamTypeForAudioUsage(usage: @AudioUsage Int): @C.StreamType Int {
        return when (usage) {
            C.USAGE_MEDIA, C.USAGE_GAME, C.USAGE_ASSISTANCE_NAVIGATION_GUIDANCE -> C.STREAM_TYPE_MUSIC
            ...
            C.USAGE_NOTIFICATION, C.USAGE_NOTIFICATION_COMMUNICATION_REQUEST, C.USAGE_NOTIFICATION_COMMUNICATION_INSTANT, C.USAGE_NOTIFICATION_COMMUNICATION_DELAYED, C.USAGE_NOTIFICATION_EVENT -> C.STREAM_TYPE_NOTIFICATION
            C.USAGE_ASSISTANCE_ACCESSIBILITY, C.USAGE_ASSISTANT, C.USAGE_UNKNOWN -> C.STREAM_TYPE_DEFAULT
            else -> C.STREAM_TYPE_DEFAULT
        }
    }
    ...
    fun getDrmUuid(drmScheme: String?): UUID? {
        return when (Ascii.toLowerCase(drmScheme)) {
    ...
    fun getErrorCodeForMediaDrmErrorCode(
        ...
        return when (mediaDrmErrorCode) {
            ...
            MediaDrm.ErrorCodes.ERROR_LICENSE_POLICY, MediaDrm.ErrorCodes.ERROR_INSUFFICIENT_OUTPUT_PROTECTION, MediaDrm.ErrorCodes.ERROR_INSUFFICIENT_SECURITY, MediaDrm.ErrorCodes.ERROR_KEY_EXPIRED, MediaDrm.ErrorCodes.ERROR_KEY_NOT_LOADED -> PlaybackException.Companion.ERROR_CODE_DRM_DISALLOWED_OPERATION
            MediaDrm.ErrorCodes.ERROR_INIT_DATA, MediaDrm.ErrorCodes.ERROR_FRAME_TOO_LARGE -> PlaybackException.Companion.ERROR_CODE_DRM_CONTENT_ERROR
            else -> PlaybackException.Companion.ERROR_CODE_DRM_SYSTEM_ERROR
        }
    }
     ...
     */
    fun inferContentType(uri: Uri): @C.ContentType Int {
        val scheme = uri.scheme
        ...
        val ismMatcher = ISM_PATH_PATTERN.matcher(Assertions.checkNotNull(uri.path))
    ...
    fun inferContentType(fileName: String): @C.ContentType Int {
     ...
     * @return The content type.
     */
    fun inferContentTypeForExtension(fileExtension: String?): @C.ContentType Int {
        var fileExtension = fileExtension
        fileExtension = Ascii.toLowerCase(fileExtension)
        return when (fileExtension) {
            "mpd" -> C.CONTENT_TYPE_DASH
            "m3u8" -> C.CONTENT_TYPE_HLS
            "ism", "isml" -> C.TYPE_SS
    ...
    fun inferContentTypeForUriAndMimeType(
        ...
        return when (mimeType) {
            MimeTypes.APPLICATION_MPD -> C.CONTENT_TYPE_DASH
            MimeTypes.APPLICATION_M3U8 -> C.CONTENT_TYPE_HLS
            MimeTypes.APPLICATION_SS -> C.CONTENT_TYPE_SS
            MimeTypes.APPLICATION_RTSP -> C.CONTENT_TYPE_RTSP
            else -> C.CONTENT_TYPE_OTHER
        }
    }
     ...
     */
    fun getAdaptiveMimeTypeForContentType(contentType: @C.ContentType Int): String? {
        return when (contentType) {
            ...
            C.CONTENT_TYPE_RTSP, C.CONTENT_TYPE_OTHER -> null
            else -> null
        }
    }
    ...
    fun unescapeFileName(fileName: String): String? {
        ...
        while (percentCharacterCount > 0 && matcher.find()) {
            val unescapedCharacter = Assertions.checkNotNull(matcher.group(1))!!.toInt(16).toChar()
    ...
    fun createTempFile(context: Context, prefix: String?): File {
        return File.createTempFile(prefix, null, Assertions.checkNotNull(context.cacheDir))
    }
     ...
     */
    val systemLanguageCodes: Array<String?>
        get() {
    ...
    val defaultDisplayLocale: Locale
        get() = if (SDK_INT >= 24) Locale.getDefault(Locale.Category.DISPLAY) else Locale.getDefault()
    ...
    fun inflate(
        ...
        inflater.setInput(input.data, input.position, input.bytesLeft())
        try {
            var outputSize = 0
            while (true) {
    ...
    fun getTrackTypeString(trackType: @TrackType Int): String {
        return when (trackType) {
            ...
            else -> if (trackType >= C.TRACK_TYPE_CUSTOM_BASE) "custom ($trackType)" else "?"
        }
    }
    ...
    fun getErrorCodeFromPlatformDiagnosticsInfo(diagnosticsInfo: String?): Int {
        ...
        return try {
            val errorCode = Assertions.checkNotNull(digitsSection)!!.toInt()
            if (isNegative) -errorCode else errorCode
        } catch (e: NumberFormatException) {
            0
        }
    }
     ...
     */
    fun getFormatSupportString(formatSupport: @C.FormatSupport Int): String {
        return when (formatSupport) {
            C.FORMAT_HANDLED -> "YES"
            C.FORMAT_EXCEEDS_CAPABILITIES -> "NO_EXCEEDS_CAPABILITIES"
            C.FORMAT_UNSUPPORTED_DRM -> "NO_UNSUPPORTED_DRM"
            C.FORMAT_UNSUPPORTED_SUBTYPE -> "NO_UNSUPPORTED_TYPE"
