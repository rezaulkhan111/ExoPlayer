...
class ThumbRating : Rating {
     ...
     */
    constructor(isThumbsUp: Boolean) {
        isRated = true
        this.isThumbsUp = isThumbsUp
    }

    public override fun hashCode(): Int {
        return Objects.hashCode(isRated, isThumbsUp)
    }

    public override fun equals(obj: Any?): Boolean {
        if (!(obj is ThumbRating)) {
            return false
        }
        val other: ThumbRating = obj
        return isThumbsUp == other.isThumbsUp && isRated == other.rated
    }

    @Documented
    @Retention(RetentionPolicy.SOURCE)
    @Target(TYPE_USE)
    @IntDef([Rating.Companion.FIELD_RATING_TYPE, ThumbRating.Companion.FIELD_RATED, ThumbRating.Companion.FIELD_IS_THUMBS_UP])
    private annotation class FieldNumber constructor()

    public override fun toBundle(): Bundle {
        val bundle: Bundle = Bundle()
        bundle.putInt(ThumbRating.Companion.keyForField(Rating.Companion.FIELD_RATING_TYPE), ThumbRating.Companion.TYPE)
        bundle.putBoolean(ThumbRating.Companion.keyForField(ThumbRating.Companion.FIELD_RATED), isRated)
        bundle.putBoolean(ThumbRating.Companion.keyForField(ThumbRating.Companion.FIELD_IS_THUMBS_UP), isThumbsUp)
        return bundle
    }

    companion object {
        // Bundleable implementation.
        private val TYPE: @RatingType Int = Rating.Companion.RATING_TYPE_THUMB
        private val FIELD_RATED: Int = 1
        private val FIELD_IS_THUMBS_UP: Int = 2

        /** Object that can restore a [ThumbRating] from a [Bundle].  */
        val CREATOR: Bundleable.Creator<ThumbRating> = Bundleable.Creator({ bundle: Bundle? -> ThumbRating.Companion.fromBundle(bundle) })
        private fun fromBundle(bundle: Bundle): ThumbRating {
            Assertions.checkArgument((
                    bundle.getInt(ThumbRating.Companion.keyForField(Rating.Companion.FIELD_RATING_TYPE),  /* defaultValue= */Rating.Companion.RATING_TYPE_UNSET)
                            == ThumbRating.Companion.TYPE))
            val rated: Boolean = bundle.getBoolean(ThumbRating.Companion.keyForField(ThumbRating.Companion.FIELD_RATED),  /* defaultValue= */false)
            return if (rated) ThumbRating(
                    bundle.getBoolean(ThumbRating.Companion.keyForField(ThumbRating.Companion.FIELD_IS_THUMBS_UP),  /* defaultValue= */false)) else ThumbRating()
        }

        private fun keyForField(field: @ThumbRating.FieldNumber Int): String {
