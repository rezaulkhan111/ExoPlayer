...
internal class StreamVolumeManager(context: Context, eventHandler: Handler, listener: Listener) {
    ...
    init {
        ...
        audioManager = checkStateNotNull(
        ...
        val receiver: VolumeChangeReceiver = VolumeChangeReceiver()
        val filter = IntentFilter(VOLUME_CHANGED_ACTION)
        try {
            registerReceiverNotExported(applicationContext, receiver, filter)
            this.receiver = receiver
        } catch (e: RuntimeException) {
            w(TAG, "Error registering stream volume receiver", e)
        }
    }

    /** Sets the audio stream type.  */
    fun setStreamType(@C.StreamType streamType: Int) {
    ...
    fun setMuted(muted: Boolean) {
        if (Util.SDK_INT >= 23) {
            audioManager!!.adjustStreamVolume(
                    streamType, if (muted) AudioManager.ADJUST_MUTE else AudioManager.ADJUST_UNMUTE, VOLUME_FLAGS)
        } else {
    ...
    fun release() {
        if (receiver != null) {
            try {
                ...
                w(TAG, "Error unregistering stream volume receiver", e)
            }
            receiver = null
        }
    }
    ...
    companion object {
        ...
        private fun getVolumeFromManager(audioManager: AudioManager?, @C.StreamType streamType: Int): Int {
            ...
            return try {
                ...
                w(TAG, "Could not retrieve stream volume for stream type $streamType", e)
                audioManager!!.getStreamMaxVolume(streamType)
            }
        }

        private fun getMutedFromManager(
                audioManager: AudioManager?, @C.StreamType streamType: Int): Boolean {
