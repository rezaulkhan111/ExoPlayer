...
interface AdViewProvider {
    /**
     * Returns the [ViewGroup] on top of the player that will show any ad UI, or `null` if
     * playing audio-only ads. Any views on top of the returned view group must be described by [ ] returned by [.getAdOverlayInfos], for accurate
     * viewability measurement.
     */
    fun getAdViewGroup(): ViewGroup?

    /**
     * Returns a list of [AdOverlayInfo] instances describing views that are on top of the ad
     ...
     */
    open fun getAdOverlayInfos(): MutableList<AdOverlayInfo?>? {
        return ImmutableList.of<AdOverlayInfo>()
    }
}